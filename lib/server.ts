...
export async function ai(path:string,body:unknown,timeout=90000):Promise<any>{
 ...
 if(!response.ok){ const problem:any=await response.json().catch(()=>({}));console.error("AI failure",response.status,problem.error?.code,problem.error?.type);if(response.status===401)throw new HttpError(503,"API 키를 인증하지 못했습니다. 전체 비밀 키로 교체해 주세요.");throw new HttpError(response.status===429?429:502,response.status===429?"AI 사용량 한도에 도달했습니다. API 잔액과 사용 한도를 확인해 주세요.":"AI 생성에 실패했습니다. 연결 설정을 확인한 후 다시 시도해 주세요.");}
